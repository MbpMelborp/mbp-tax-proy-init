...
<template>
  <UDashboardCard
    ...
    :ui="{
      ...
      body: { padding: '!pb-3 !px-0', background: 'bg-transparent' } as any,
      header: { background: 'bg-transparent' } as any,
      footer: { background: 'bg-transparent' } as any,
    }"
  >
    <template #header>
      <div>
        <p class="mb-1 text-sm font-medium text-primary-500 dark:text-primary-400">
          Revenue
        </p>
        <p class="text-3xl font-semibold text-primary-900 dark:text-white">
          {{ formatNumber(total) }}
        </p>
      </div>
    </template>
